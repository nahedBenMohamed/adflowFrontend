...
import { OnlineBookingSiteFormBuilderStep2 } from '@/modules/builder/pages/OnlineBookingSiteFormBuilderPage/components/OnlineBookingSiteFormBuilderStep2/OnlineBookingSiteFormBuilderStep2';
import {
  SiteFormBuilderStep3,
  SiteFormBuilderStep4,
  SiteFormBuilderStep5,
} from '@/modules/builder/pages/SiteFormBuilderPage/components';
import {
  envUtil,
  useErrorMessageIdle,
  useTitle,
...
import {
  BuilderStepControls,
  BuilderTabs,
  generateOnlineBookingSiteFormBuilderNavSteps,
...
import { BuilderNavStore, OnlineBookingSiteFormBuilderStore } from '../../store';
import { BuilderWithHorizontalNavPageTemplate } from '../../templates';
import { OnlineBookingSiteFormBuilderStep1 } from './components';
...
const OnlineBookingSiteFormBuilder = observer(() => {
  const { t } = useTranslation('module.builder', {
    keyPrefix: 'builder.pages.online_booking_site_form_builder_page.steps',
  });
  ...
  const siteFormBuilderStore = useMemo(
    () =>
      new OnlineBookingSiteFormBuilderStore({
  ...
  const siteFormBuilderNavSteps = useMemo<BuilderNavStep[]>(
    () => generateOnlineBookingSiteFormBuilderNavSteps(t),
    [t]
  );
  ...
  useLayoutEffect(() => {
    ...
    else if (appStore.isLoaded) loadSchedules();
  ...
  const nextProps = useMemo<Optional<BuilderStepControlsButtonProps>>(() => {
    ...
    if (activeStepOrder === 1 && !siteFormElementsFormData.pages[0]?.fields.length)
      return {
        onClick: (): void => {
          siteFormElementsFormData.initializeDefaultFormElementsData(t);
      ...
      <Root $withoutPadding={activeStepOrder === 4}>
        {activeStepOrder === 1 && (
          <HorizontalStepMotion motionKey={1} direction={motionDirection}>
            <OnlineBookingSiteFormBuilderStep1
              loading={loading}
              siteFormBuilderStore={siteFormBuilderStore}
            />
          </HorizontalStepMotion>
        )}
        ...
        {activeStepOrder === 2 && (
          <HorizontalStepMotion motionKey={2} direction={motionDirection}>
            <OnlineBookingSiteFormBuilderStep2
...
OnlineBookingSiteFormBuilder.displayName = 'OnlineBookingSiteFormBuilder';
export { OnlineBookingSiteFormBuilder };
